...
public class JokeManager {
    ...
    private static String getXmlString(String targetLang, String key) {
        try {
            String effLang = getEffectiveLang(targetLang);
            File file = "values".equals(effLang) ? 
                new File("src/main/res_joke/values/strings_joke.xml") : 
                new File("src/main/res_joke/values-" + effLang + "/strings_joke.xml");
            if (!file.exists()) file = new File("src/main/res_joke/values/strings_joke.xml");
            if (file.exists()) {
    ...
    public static String handleSubstitutions(Context context, String userText, String userLanguage) {
        ...
        if (cleanedText.contains("хақорат") || cleanedText.contains("мат") || cleanedText.contains("безеб")) {
            ...
            String warn = getXmlString(userLanguage, "prank_warning_bad_word");
            ...
            if (file.exists()) {
                ...
                while (eventType != XmlPullParser.END_DOCUMENT) {
                    if (eventType == XmlPullParser.START_TAG && "string".equals(parser.getName())) {
                        ...
                        if (nameAttr != null && nameAttr.startsWith("joke_c") && nameAttr.endsWith("_desc")) {
                            jokesList.add(parser.nextText());
                        }
                    }
                    eventType = parser.next();
                }
    ...
    public static boolean saveUserSubstitution(Context context, final String stringKey, final String userText, final String userLanguage, boolean isPremiumUser) {
        ...
        if (!isPremiumUser) {
            if (currentCount == 70) {
                String msg = getXmlString(userLanguage, "prank_gift_msg");
                Toast.makeText(context, msg != null ? msg : "Gift!", Toast.LENGTH_LONG).show();
            }
            if (currentCount >= 100) {
                String msg = getXmlString(userLanguage, "prank_limit_free");
                Toast.makeText(context, msg != null ? msg : "Premium Required!", Toast.LENGTH_LONG).show();
                return false;
            }
        } else {
            if (currentCount >= 500) {
                String msg = getXmlString(userLanguage, "prank_limit_premium");
        ...
        if (getDatabase() != null) {
            ...
            getDatabase().child(effLang).child(stringKey).setValue(stringData);
        }
        return true;
    }

    public interface JokeCallback { void onResult(String resultText); }
}
